...
>
> 本篇的计算草稿：https://gitee.com/chaoskey/notes/blob/master/content/code/0076.ipynb
...
考虑$$T_{ab}$$是一个$$(0,2)$$型(实)张量，$$T_{AA'BB'}$$是其相关的旋量张量，容易直接验证一个恒等式：
$$
\boxed{\frac{1}{2}(T_{AA'BB'}-T_{BB'AA'})=T_{(AB)[A'B']}+T_{[AB](A'B')}}
$$
然后，可直接构造两个对称旋量张量：
$$
\phi_{AB}=\phi_{(AB)}\overset{\Delta}{=}\frac{1}{2}T_{(AB)A'}^{\ \qquad A'}\\ \psi_{A'B'}=\psi_{(A'B')}\overset{\Delta}{=}\frac{1}{2}T_{(A'B')A}^{\ \qquad A}
$$
由于$$T_{ab}$$进而$$T_{AA'BB'}$$的实数性，意味着这两个构造并不独立：$$\bar{\psi}_{AB}=\phi_{AB}$$。

进而：
$$
T_{(AB)[A'B']}=\phi_{AB}\bar{\epsilon}_{A'B'}\\ T_{[AB](A'B')}=\epsilon_{AB}\psi_{A'B'}=\epsilon_{AB}\bar{\phi}_{A'B'}
$$
于是，前面的恒等式，可表示成：
$$
\boxed{\frac{1}{2}(T_{AA'BB'}-T_{BB'AA'})=\phi_{AB}\bar{\epsilon}_{A'B'}+\epsilon_{AB}\bar{\phi}_{A'B'}}
$$
这意味着，每个反称张量$$T_{ab}=T_{[ab]}$$关联的旋量张量都可以写成上式右边的形式。

类似的，我们有：
$$
\boxed{\begin{aligned}\frac{1}{2}(T_{AA'BB'}+T_{BB'AA'})&=T_{(AB)(A'B')}+T_{[AB][A'B']}\\ &=T_{(AB)(A'B')}+\frac{1}{4}\epsilon_{AB}\bar{\epsilon}_{A'B'} T \end{aligned}} \\ T\overset{\Delta}{=}T_{A\ \ A'}^{\ \ A\ \ A'}=-T_a^{\ \ a}
$$
这意味着，每个对称张量$$T_{ab}=T_{(ab)}$$关联的旋量张量都可以写成上式右边的形式（方框中）。

上述公式的推导过程中，会有一个有用中间结论：

$$
\boxed{T_{[AB]A'}^{\qquad A'}=\frac{1}{2}\epsilon_{AB}T}
$$

如果$$T_{ab}$$是对称的，那么这个公式左边的方括号可以忽略。
