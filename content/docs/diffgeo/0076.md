...
$$
\boxed{\xi_A=\xi^B\epsilon_{BA},\quad \xi^A=\epsilon^{AB}\xi_B\\ \epsilon^{AB}\overset{\Delta}{=}(\epsilon_{AB})^{-1}\quad \epsilon_{AB}=\epsilon^{CD}\epsilon_{CA}\epsilon_{DB}}
$$
如图考虑两条映射路径：

![](../images/0125.svg)

紫色所示映射，有两种表示方式：红色复合映射 和 蓝色复合映射。【这个映射图利用了逆协变旋量定义和期待中的旋量度规降指标功能】即：
$$
...
$$
\boxed{\epsilon_{AB}=-\epsilon_{BA}\quad \epsilon^{AB}\epsilon_{BC}=-\delta^A_C\\ \quad \\ \epsilon_{\Sigma\Omega}=\begin{pmatrix} 0 & 1 \\  -1& 0 \end{pmatrix}\quad\epsilon^{\Sigma\Omega}=\begin{pmatrix} 0 & 1 \\  -1& 0 \end{pmatrix}}
$$
...
$$
\boxed{\xi_A\phi^A=-\xi^A\phi_A\quad \xi_A\xi^A=0}
$$

不妨用$$\epsilon^{\ \ A}_{C}$$（代替容易混淆的$$\delta^A_C$$）来表示$$W$$和$$W^*$$上的恒等算符：

$$
\boxed{\epsilon^{\ \ A}_{C}=-\epsilon^{A}_{\ \ C}\overset{\Delta}{=}\delta^A_C\\ \quad \\ \epsilon^{\ \ A}_{A}=2=-\epsilon^{A}_{\ \ A}}
$$
...
有：
$$
\boxed{\bar{\epsilon}_{A'B'}=-\bar{\epsilon}_{B'A'}\quad \bar{\epsilon}^{A'B'}\bar{\epsilon}_{B'C'}=-\delta^A_C\\ \quad \\ \bar{\epsilon}_{\Sigma'\Omega'}=\begin{pmatrix} 0 & -1 \\  1 & 0 \end{pmatrix}\quad \bar{\epsilon}^{\Sigma'\Omega'}=\begin{pmatrix} 0 & -1 \\  1 & 0 \end{pmatrix}}
$$
进而，$$\bar{\epsilon}_{A'B'}$$和$$\bar{\epsilon}^{A'B'}$$也有类似的功效【右下左上缩并】：
$$
\boxed{\bar{\epsilon}_{A'B'}\bar{L}^{\ \ A'}_{C'}\bar{L}^{\ \ B'}_{D'}=\bar{\epsilon}_{C'D'}\\ \quad \\ \eta_{A'}=\eta^{B'}\bar{\epsilon}_{B'A'},\quad \eta^{A'}=\bar{\epsilon}^{A'B'}\eta_{B'}\\ \bar{\epsilon}^{A'B'}=(\bar{\epsilon}_{A'B'})^{-1}\quad \bar{\epsilon}_{A'B'}=\bar{\epsilon}^{C'D'}\bar{\epsilon}_{C'A'}\bar{\epsilon}_{D'B'}\\ \quad \\ \bar{\epsilon}^{\ \ A'}_{C'}=-\bar{\epsilon}^{A'}_{\ \ C'}=\delta^{A'}_{C'}\quad  \bar{\epsilon}^{\ \ A'}_{A'}=2=-\bar{\epsilon}^{A'}_{\ \ A'}}
$$
