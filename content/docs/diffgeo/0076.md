...
考虑任意一个形如$$T^{\dots}_{\quad \dots AA'\dots BB'\dots }$$的旋量张量，为简单起见，用$$T_{AA'BB'}$$表示之，可直接验证一个恒等式【右边展开后合并易得】：
$$
...
类似的，我们有：
$$
\boxed{\frac{1}{2}(T_{AA'BB'}+T_{BB'AA'})=T_{(AB)(A'B')}+\frac{1}{4}\epsilon_{AB}\bar{\epsilon}_{A'B'} T} \\ T\overset{\Delta}{=}T_{A\ \ A'}^{\ \ A\ \ A'}=-T_a^{\ \ a}
$$
【结论】：对称张量$$T_{ab}=T_{(ab)}$$对应的旋量张量（局部对称也行），可以按上面的方式分解。

上面这个公式的推导过程中，会有一个有用中间结论：

$$
\boxed{T_{[AB]A'}^{\qquad A'}=\frac{1}{2}\epsilon_{AB}T}
$$

特别地，如果$$T_{ab}$$是对称的，那么这个公式左边的方括号可以忽略。
