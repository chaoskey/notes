...
>
> 旋量概念源自$$\mathrm{SU}(2)$$对$$\mathrm{SO}(3)$$是双重覆盖，将$$\mathrm{SU}(2)$$中表示旋转张量“劈”为两半，其中一半就是旋量空间上的逆变旋量，另一半是其共轭空间的共轭逆变旋量。
>
...
>
> 关于旋量运算的约定：1）约定指标左上右下缩并；2）泡利矩阵前加因子$$1/\sqrt{2}$$。
...
$$
\begin{aligned}t'&=(\cosh\alpha)t+(\sinh\alpha)(\boldsymbol{n}_\alpha\cdot\boldsymbol{x})\\ \boldsymbol{x}'&=\left[(\sinh\alpha)t+(\cosh\alpha)(\boldsymbol{n}_\alpha\cdot\boldsymbol{x})\right]\boldsymbol{n}_\alpha\quad \text{【与}\boldsymbol{n}_\alpha\text{相平行部分不发生变换】}\\&\qquad+\left[\boldsymbol{x}-(\boldsymbol{n}_\alpha\cdot\boldsymbol{x})\boldsymbol{n}_\alpha\right]\quad \text{【与}\boldsymbol{n}_\alpha\text{相垂直部分不变】}\end{aligned}
$$
...
$$
g_{ab}=\delta_{ab}\quad \delta_{\mu\upsilon}=\mathrm{diag}(1,1,1) \quad \text{【三维欧氏空间】} \\ g_{ab}=\eta_{ab}\quad \delta_{\mu\upsilon}=\mathrm{diag}(-1,1,1,1) \quad \text{【四维闵氏时空】}
$$
...
这就是前面提到到的：$$\mathrm{SU}(2)$$对$$\mathrm{SO}(3)$$是双重覆盖。为了更显式描绘这个特征，我们抽取出空间（或 时空）旋转变换的一半，用一个二分量的量表示之，称为`旋量`。旋量的变换规律遵循如下规律：

$$
\xi\mapsto\xi'=R\xi\quad \text{【三维欧氏空间】}\\ \xi\mapsto\xi'=L\xi\quad \text{【四维闵氏时空】}
$$

这个量由于没有抵消因子，当旋转$$2\pi$$时，$$\xi$$显式出现反向特征。

很明显，旋量$$\xi$$属于二维复矢量空间（记作$$W$$），称之为`逆变旋量`。用大写拉丁字母作为上标来标记$$W$$的元素，比如：$$\xi^A\in W$$，这是一个坐标无关的量。对应用大写希腊字母为上标来标记$$\xi^A$$的分量，比如：$$\xi^\Sigma$$。【引入旋量的抽象指标记号】

首先，有$$W$$的`对偶空间`$$W^*$$，其元素则对应记作（比如）$$\xi_A\in W^*$$，称之为`协变旋量`，这是$$W\to \mathbb{C}$$的`线性`映射。

然后，有$$W^*$$的`共轭空间`$$\overline{W}^*$$(即，$$W$$的`共轭对偶空间`)，其元素记作$$\xi_{A'}\in \overline{W}^*$$，称之为`共轭协变旋量`，这是$$W\to \mathbb{C}$$的`反线性`映射。

最后，有的$$\overline{W}^*$$`对偶空间`$$\overline{W}$$（即，$$W$$的`共轭空间`）,其元素记作$$\xi^{A'}\in \overline{W}$$，称之为`共轭逆变旋量`。

根据上面的符号约定，我们有如下关系【厄米的】：

$$
\overline{\xi^A}=\overline{\xi}^{A'},\quad \overline{\phi_A}=\overline{\phi}_{A'}
$$
...
$$(k,l;k',l')$$型`旋张量`$$T$$的`共轭`$$\overline{T}$$是$$(k',l';k,l)$$型`旋量张量`【厄米的】，比如：
...
对二维复矢量空间$$W$$而言，所有(0,2;0,0)型反称张量构成的矢量空间是一维的。如果从中选择一个非退化的（类似度规的东西，可看作一个非退化映射，见后）

$$
\epsilon_{AB}=-\epsilon_{BA}
$$

于是称$$(W,\epsilon_{AB})$$为`旋量空间`，其中每个元素成为`旋量`，其上的张量则是`旋量张量`（或 `高阶旋量`）。

使用$$\epsilon_{AB}$$可以得到一个旋量到对偶旋量的映射$$\xi^A\mapsto\epsilon_{AB}\xi^A$$，进而获得一个同构映射$$W\to W^*$$。于是元素$$\xi_A\in W^*$$可表示成

$$
\xi_B=\xi^A\epsilon_{AB}=-\xi^A\epsilon_{BA}
$$

可用$$\epsilon_{AB}$$的负逆映射来定义$$\epsilon^{AB}$$【$$\delta^A_{\ \ C}$$是$$W$$上的恒等映射】

$$
\epsilon^{AB}\epsilon_{BC}=-\delta^A_C
$$

于是，元素$$\xi^A\in W$$反而可表示成

$$
\xi^A=\epsilon^{AB}\xi_B=-\epsilon^{BA}\xi_B
$$
...
【注意: 参与缩并的两项，若带着指标整体交换是不变号，但若仅仅是一对上标下标交换是会变号的】

不妨用$$\epsilon^{\ \ A}_{C}$$（代替容易混淆的$$\delta^A_C$$）来表示$$W$$和$$W^*$$上的恒等算符：

$$
\epsilon^{\ \ A}_{C}=-\epsilon^{A}_{\ \ C}=\delta^A_C\\ \quad \\ \epsilon^{\ \ A}_{A}=2=-\epsilon^{A}_{\ \ A}
$$

正如可用$$\epsilon_{AB}$$和$$\epsilon^{AB}$$对不带撇的指标进行下降和提升作用。也可以用对应的$$\bar\epsilon_{A'B'}$$和$$\bar{\epsilon}^{A'B'}$$对带撇的指标进行下降和提升作用。

最后，可以用$$\epsilon$$来表示变换$$L^A_{\ \ C}$$的行列式【第二个式子意味着保旋量内积】

$$
\det(L)=\frac{1}{2}\epsilon_{AB}\epsilon^{CD}L^A_{\ \ C}L^B_{\ \ D}\\ \quad \\ \det(L)=1\Longleftrightarrow \epsilon_{AB}L^A_{\ \ C}L^B_{\ \ D}=\epsilon_{CD}
$$
...
类似地，也可写出$$\epsilon_{AB}$$分量形式$$\epsilon_{\Sigma\Omega}$$：

$$
\epsilon^{\Sigma\Omega}=\begin{pmatrix} \epsilon^{AB}\iota_A\iota_B & -\epsilon^{AB}\iota_A\omicron_B \\ -\epsilon^{AB}\omicron_A\iota_B & \epsilon^{AB}\omicron_A\omicron_B \end{pmatrix}=\begin{pmatrix} 0 & 1 \\ -1 & 0 \end{pmatrix}\\ \epsilon_{\Sigma\Omega}=\begin{pmatrix} \epsilon_{AB}\iota^A\iota^B & -\epsilon_{AB}\iota^A\omicron^B \\ -\epsilon_{AB}\omicron^A\iota^B & \epsilon_{AB}\omicron^A\omicron^B \end{pmatrix}=\begin{pmatrix} 0 & 1 \\ -1 & 0 \end{pmatrix}
$$
...
$$
\begin{aligned}X&\mapsto RXR^\dagger\quad \text{【三维欧氏空间】}\\ X&\mapsto LXL^\dagger\quad \text{【四维闵氏时空】} \end{aligned}
$$

改写称抽象指标形式【以后若无特别说明，结论通用于三维欧氏空间和四维闵氏时空】
...
若希望能用旋量恢复成完整旋转变换，则需补上另一半：

$$
\eta^{A'}=\bar{L}^{A'}_{\ \ B'}\eta^{B'}
$$
于是有：
...
$$
T_{AA'}^{\quad BB'CC'}=\sigma^\alpha_{\ \ AA'}\sigma_\beta^{\ \ BB'}\sigma_\gamma^{\ \ CC'} T_\alpha^{\ \ \beta\gamma} \\ T_\alpha^{\ \ \beta\gamma}=(-1)^3\sigma_\alpha^{\ \ AA'}\sigma^\beta_{\ \ BB'}\sigma^\gamma_{\ \ CC'}T_{AA'}^{\quad BB'CC'}
$$
