...
$$
(\xi^\Sigma)=\begin{pmatrix} \xi^1 \\ \xi^2  \end{pmatrix}=\begin{pmatrix} \sqrt{2r}\cos\frac{\theta}{2}\ \exp(\frac{-i(\phi-\alpha)}{2}) \\ \sqrt{2r}\sin\frac{\theta}{2}\ \exp(\frac{i(\phi+\alpha)}{2})   \end{pmatrix}
$$

旋量$$\eta^A$$可由$$\xi^A$$确定（精确到可加一个$$\xi^A$$的实倍数）：

$$
(\eta^\Sigma)=\begin{pmatrix} \eta^1 \\ \eta^2  \end{pmatrix}=\begin{pmatrix} -\frac{1}{\sqrt{2r}}\sin\frac{\theta}{2}\ \exp(\frac{-i(\phi+\alpha)}{2}) \\ \frac{1}{\sqrt{2r}}\cos\frac{\theta}{2}\ \exp(\frac{i(\phi-\alpha)}{2})   \end{pmatrix}
$$

进而：

$$
l^\mu\sigma_\mu^{\ \ AA'}= l^{AA'}=\xi^A\bar{\eta}^{A'}+\eta^A\bar{\xi}^{A'}\\ \text{改写成分量形式}\\ \begin{pmatrix} l^0+l^3 & l^1-il^2 \\ l^1+il^2 & l^0-l^3 \end{pmatrix} =\begin{pmatrix} \xi^1\bar{\eta}^{1'} & \xi^1\bar{\eta}^{2'} \\ \xi^2\bar{\eta}^{1'} & \xi^2\bar{\eta}^{2'} \end{pmatrix}+\begin{pmatrix} \eta^1\bar{\xi}^{1'} & \eta^1\bar{\xi}^{2'} \\ \eta^2\bar{\xi}^{1'} & \eta^2\bar{\xi}^{2'} \end{pmatrix}
$$

将旋量分量表达式代入，并解出四维矢量$$l^a$$的所有分量：

$$
(l^\mu)=\begin{pmatrix} l^0 \\ l^1 \\ l^2 \\ l^3  \end{pmatrix}=\begin{pmatrix} 0 \\ \cos\theta\cos\phi\cos\alpha+\sin\phi\sin\alpha \\ \cos\theta\sin\phi\cos\alpha-\cos\phi\sin\alpha \\ -\sin \theta \cos \alpha  \end{pmatrix}
$$
