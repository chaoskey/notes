...
旋量是度规依赖的，我们只能在（伪）黎曼流形$$(M,g_{ab})$$上考虑，并且始终选定正交归一标架场【约定$$(-1,1,1,1)$$度规】。 那么，对张量场而言，存在与度规$$g_{ab}$$唯一适配的的导数算符$$\nabla_a$$。 

进而，由于$$\nabla_a$$很像“协变矢量”，我们可将这个“协变矢量”关联的“旋量张量”$$\nabla_{AA'}$$定义为`旋量场的导数算符`：
...
$$
\nabla_{AA'}f=\sigma^a_{\ \ AA'}\nabla_a f
$$

对任意给定逆变矢量$$v^a$$，其关联着旋量张量$$v^{AA'}$$，有：
...
$$
\nabla_{AA'}\nabla_{BB'}f=\sigma^a_{\ \ AA'}\sigma^b_{\ \ BB'}\nabla_a\nabla_b f=\sigma^b_{\ \ BB'}\sigma^a_{\ \ AA'}\nabla_b\nabla_a f=\nabla_{BB'}\nabla_{AA'}f
$$

所以，旋量场导数算符$$\nabla_{AA'}$$，还满足:

- 4）可退化性：$$v(f)=-v^{AA'}\nabla_{AA'}f\quad \forall f \in \mathscr{F}_M, v^a\in \mathscr{F}_M(1,0) $$
- 5）无挠性：$$\nabla_{AA'}\nabla_{BB'}f=\nabla_{BB'}\nabla_{AA'}f\quad \forall f\in \mathscr{F}_M$$
...
黎曼曲率张量$$R_{abc}^{\quad d}$$的定义式改写成旋量形式:

$$
(\nabla_{AA'}\nabla_{BB'}-\nabla_{BB'}\nabla_{AA'})\omega_{CC'}=R_{AA'BB'CC'}^{\qquad \qquad DD'}\omega_{DD'}
$$

类似地，`曲率旋量`$$\chi_{AA'BB'C}^{\quad \qquad D}$$的定义式为：
...
很明显，$$\epsilon^{AC}\Lambda_{ABCD}$$是反称的，必定有一个乘子$$\epsilon_{BD}$$。 于是可以定义$$\Psi_{ABCD}$$，我们将会看见，这就是`Weyl旋量`：
...
根据第一篇末尾得恒等式，旋量场导数算符对易子，可分解成：

$$
\boxed{\nabla_{AA'}\nabla_{BB'}-\nabla_{BB'}\nabla_{CC'}=\bar{\epsilon}_{A'B'}\Delta_{AB}+\epsilon_{AB}\Delta_{A'B'}}\\  \Delta_{AB}\overset{\Delta}{=}\nabla_{X'(A}\nabla_{B)}^{\quad X'}
$$
...
$$
\boxed{\begin{aligned}\Delta_{A'B'}\omega_C&=\Phi_{A'B'C}^{\qquad D}\omega_D \\ &\quad \\ \Delta_{AB}\omega_C&=\Lambda_{ABC}^{\ \ \quad D}\omega_D\\ &=\Psi_{ABC}^{\ \ \quad D}\omega_D-2\Lambda\epsilon_{C(A}\omega_{B)}\end{aligned}}
$$
