...
$$
我们知道$$R_{abcd}$$关于最后两个指标是反称的，蕴含着（根据上式），$$\chi_{AA'BB'CD}$$关于最后两个指标是对称的。

我们还是知道$$R_{abcd}$$关于头两个指标是反称的，蕴含着，$$\chi_{AA'BB'CD}$$可表示成如下形式【见第一篇末尾“几个有用的恒等式”】：
$$
...
$$
很明显，$$\epsilon^{AC}\Lambda_{ABCD}$$是反称的，进而必定有一个乘子$$\epsilon_{BD}$$。 于是可以定义$$\Psi_{ABCD}$$：
$$
...
$$
如此构造的$$\Psi_{ABCD}$$，必定满足
$$
\epsilon^{AC}\Psi_{ABCD}=0
$$
再与$$\Lambda_{ABCD}$$的对称性一起，蕴含着，$$\Psi_{ABCD}$$是全对称的【？？？？？】
$$
\Psi_{ABCD}=\Psi_{(ABCD)}
$$
还有，352   371
