...
闵氏时空上旋量场的导数可以按如下方式定义。因为一个旋量场$$\psi^A$$是时空点到$$W$$的映射，可在闵氏时空上关于全局惯性坐标系下，定义$$\psi^A$$的普通偏导数$$\partial_{BB'}\psi^A$$，这是一个(1,1;0,1)型旋量张量场，在基底$$\{\omicron^A,\iota^A\}$$下，其分量是：
$$
...
$$
#### 三）用黎曼曲率张量来表示”类曲率“旋量张量

由于$$R_{abcd}$$关于最后两个指标是反称的，依据第一篇末尾的恒等式，有
$$
...
$$
\boxed{\chi_{AA'BB'CD}=\frac{1}{2}R_{AA'BB'(CD)C'}^{\quad \qquad \qquad C'}}
$$
这说明，$$\chi_{AA'BB'CD}$$关于最后两个指标是对称的：
$$
\boxed{\chi_{AA'BB'CD}=\chi_{AA'BB'(CD)} \quad  \chi_{AA'BB'[CD]}=0}
$$

#### 四）”类曲率“旋量张量的分解

还是知道$$R_{abcd}$$关于头两指标是反称的，再次依据第一篇末尾的恒等式，有：
$$
\boxed{\chi_{AA'BB'CD}=\Lambda_{ABCD}\bar{\epsilon}_{A'B'}+\Phi_{A'B'CD}\epsilon_{AB}}\\ \Lambda_{ABCD}=\Lambda_{(AB)(CD)}=\frac{1}{2}\chi^{\ \qquad A'}_{(AB)A'\ \ CD}\\ \Phi_{A'B'CD}=\Phi_{(A'B')(CD)}=\frac{1}{2}\chi^{\ \ \qquad A}_{(A'B')A\ \ CD}
$$
此外，$$R_{abcd}=R_{cdab}$$，还蕴含着【需要经过强算】：
$$
\begin{aligned}\bar{\Phi}_{ABC'D'}&=\Phi_{ABC'D'} \quad \text{实数性}\\ \quad \\ \Lambda_{ABCD}&=\Lambda_{CDAB}\end{aligned}
$$
#### 五）黎曼曲率张量的分解

很明显，$$\epsilon^{AC}\Lambda_{ABCD}$$是反称的，必定有一个乘子$$\epsilon_{BD}$$。 于是可以定义$$\Psi_{ABCD}$$：
$$
...
$$
再与$$\Lambda_{ABCD}$$的对称性一起，蕴含着，$$\Psi_{ABCD}$$是全对称的：【我没证出来？】
$$
\Psi_{ABCD}=\Psi_{(ABCD)}
$$
最后，根据$$R_{a[bcd]}=0$$，及第一篇末尾的恒等式，可知：【我没证出来？】
$$
\bar{\Lambda}=\Lambda\quad \text{实数性}
$$
因此，"类曲率"旋量张量可以分解乘：
$$
\chi_{AA'BB'CD}=\Psi_{ABCD}\bar{\epsilon}_{A'B'}+\Phi_{A'B'CD}\epsilon_{AB}+\Lambda(\epsilon_{AC}\epsilon_{BD}+\epsilon_{BC}\epsilon_{AD})\bar{\epsilon}_{A'B'}
$$
进而，黎曼曲率张量可分解成：
$$
...
黎曼曲率张量分解中有一部分，对应`Weyl张量`$$C_{abcd}$$：
$$
