...
2. 两个有用的关系：
   $$
   \varepsilon^{a_1\dots a_n}\varepsilon_{b_1\dots b_n}=(-1)^s n!\delta^{[a_1}_{\ \ b_1}\dots\delta^{a_n]}_{\ \ b_n} \\ \quad \\ \varepsilon^{a_1\dots a_j a_{j+1} \dots a_n}\varepsilon_{b_1\dots a_j b_{j+1} \dots b_n}=(-1)^s (n-j)!j!\delta^{[a_{j+1}}_{\ \ \quad b_{j+1}}\dots\delta^{a_n]}_{\ \ b_n}
   $$
