...
进而可抛掉被作用作用对象【其中$$\textcolor{red}{C^c_{\ \ ab}}$$是李代数$$\mathscr{G}$$的结构常数】：
...
2）$$\kappa([A,B],C)=\kappa(A,[B,C])$$；【利用方阵乘积的“迹”的可轮换性易证】
...
$$
[E_i,E_j]=\varepsilon^k_{\ \ ij}E_k\\ C^k_{\ \ ij}=\varepsilon^i_{\ \ ij}
$$
