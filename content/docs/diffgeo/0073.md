...
> 通过类比正交群掌握辛群的基本概念。
>
> `反称度规`$$s_{ab}$$，是一个2-形式。只有偶数阶，才是非奇异的。
>
> `辛群`，是保`反称度规`的线性映射集合，也是李群。
>
> 辛群的元素，作为线性映射而言，就是正则变换。
>
> `辛群的李代数`的元素，可以看成$$S$$乘上一个对称矩阵。
...
$$
s(v,u)=-s(u,v)\quad \text{映射形式}\\ s_{ab}=-s_{ab}\quad \text{张量形式} \\ S^T=-S\quad \text{矩阵形式}
$$
...
于是，类似正交群， `辛群`的元素$$T$$（`正则变换`）就是保反称度规$$s_{ab}$$的线性映射：
...
实际上，反称度规$$s_{ab}$$，作为一个反称张量，其实就2-形式。如果选择一个坐标基底$$\{(e_\mu)^a\}$$，相应有对偶基底$$\{(e^\mu)_a\}$$。 于是反称度规$$s_{ab}$$：
...
$$
\begin{aligned}s_{ab}&=\sum_{(\mu,\upsilon)\in\chi_2(1\dots m)}{s_{\mu\upsilon}(e^\mu)_a\wedge(e^\upsilon)_b} \\ &=\sum_{(\mu,\upsilon)\in\chi_2(1\dots \frac{m}{2})}{s_{\mu\upsilon}(e^\mu)_a\wedge(e^\upsilon)_b}+\sum_{\mu<=\frac{m}{2}<\upsilon}{s_{\mu\upsilon}(e^\mu)_a\wedge(e^\upsilon)_b}\\ & \qquad+\sum_{(\mu,\upsilon)\in\chi_2(\frac{m}{2}+1\dots m)}{s_{\mu\upsilon}(e^\mu)_a\wedge(e^\upsilon)_b} \\ &=\boxed{\sum_{\mu<=\frac{m}{2}<\upsilon}{\delta_{\mu(\upsilon-\frac{m}{2})}(e^\mu)_a\wedge(e^\upsilon)_b}}\end{aligned}
$$
