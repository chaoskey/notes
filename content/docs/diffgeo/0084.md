...
>
> `权矢量`，就是零根空间（嘉当子代数）所有基底的共同本征矢对应的本征根组成的矢量。
>
> 非零根标准基底对应升降算符。正根对应升算符，负根对应降算符。
>
> `权矢量`在升降算符的作用后，依然是`权矢量`，并且对应的`权`被提高或降低了。
>
> 如果某个`权矢量`被升算符作用后得零，那么该`权矢量`是最高权矢量，对于的权是最高权。
>
> 通过`Dynkin图`可衍生的不可约表示图，利用相关性质容易计算出不可约表示维度。
...
$$
\cos\varphi\ne 180^\circ \Longrightarrow \boxed{\cos^2\varphi\ne 1}\\\boldsymbol{\alpha}\cdot\boldsymbol{\beta}\le 0\Longrightarrow \boxed{\cos\varphi=\frac{\boldsymbol{\alpha}\cdot\boldsymbol{\beta}}{|\boldsymbol{\alpha}||\boldsymbol{\beta}|}<0}
$$
...
- 从`实数根`的角度看，$$A^a$$在嘉当子代数（零根空间）的不同选择，只会导致所有实数根乘上一个比例系数（可以是复数系数）。【总结：$$A^a$$的选择可能会导致整个“根矢量几何”等比放大或缩小】
