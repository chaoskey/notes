...
可通过求解Killing方程确定欧氏空间的平移群$$T(3)$$和转动群$$\mathrm{SO}(3)$$。合起来对应Killing矢量的6个基底所张成的空间。
