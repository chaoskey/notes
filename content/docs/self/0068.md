...
$$
\mathscr{O}(m)=\{A\in M(m,\mathbb{R})| A^T = -A \}\\
\dim O(m)= \dim \mathscr{O}(m)=\frac{1}{2}m(m-1)
$$
...
进而，算符$$U$$为`酉算符`的充要条件的矩阵形式：$$U^{-1}=U^\dagger=\overline{A^T}$$。  对应的矩阵$$U$$称为`酉矩阵`（或 `幺正矩阵`）。
