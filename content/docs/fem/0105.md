...
在所有这些例子中，核心概念是将给定的函数元组映射为实数的形式。  我们将这些称为多重线性形式。  下面，我们将多重线性形式的概念形式化，讨论多重线性形式的离散化，以及相关概念，例如多重线性形式的作用量，导数和伴随。
...
$$
a'(\bar{u}; \delta u, v) \equiv a'(v; \bar{u})\delta u = \int_\Omega{[(\mathrm{grad} \ \delta u \cdot \bar{u}) \cdot v + (\mathrm{grad} \ u¯ \ \delta u) \cdot v] dx} \tag{5.12}
$$
...
胞元张量$A_T$是典型的密集张量，其秩为$\rho$和维度为$n_1 \times n_2\times\dots\times n_\rho$。  离散算符A可以通过对每个胞元张量$A_T$贡献适当求和来获得。  我们将在后面的第6章中进行详细介绍。
...
我们将线性形式$\mathcal{A}a$称为双线性形式$a$的作用量。  通常，具有$n$个系数的$\rho$重线性形式的作用量是具有$n+1$个系数的$(\rho−1)$线性形式。  特别地，双线性形式的作用量是线性形式，而线性形式的作用量是函数。

双线性形式的作用量在求解微分方程的无矩阵法的定义中起着重要作用。  考虑通过Krylov子空间方法（例如GMRES（广义最小残差方法）（Saad和Schultz，1986）或CG（共轭梯度方法）（Hestenes和Stiefel，1952））求解正则形式（5.1）的变分问题。  Krylov方法，通过由向量$b, A b, A^2 b, \dots, A^k b\quad k \ll N$张成的$\mathbb{R}^N$子空间寻求$U$的近似，来获得线性方程组$A U = b$的近似解$U \in \mathbb{R}^N$。  这些向量可以通过重复应用前面定义的离散算符$A$来计算：
...
其中，$u_h = \sum^N_{j=1}U_j\phi^2_j$是对应于系数向量$U$的有限元近似。  换句话说，矩阵$A$在给定向量$U$上的应用，是由相应的有限元近似下双线性形式取值的作用量给出：

$$
(AU)_i = (\mathcal{A}a)(u_h; \phi^1_i) \tag{5.24}
$$

因此，可以通过对线性形式（双线性形式$a$的作用量$\mathcal{A}a$）的计算（组装）来求解变分问题（5.1），以代替先计算（组装）矩阵$A$，后重复计算与$A$的矩阵-矢量积。  哪种方法更有效取决于与矩阵组装相比作用量的计算效率，以及可用的预处理器。  对于多重线性形式作用量的进一步讨论，参考Bagheri和Scott（2004）。

UFL形式语言通过调用`action`函数来支持计算多重线性形式的作用量：
