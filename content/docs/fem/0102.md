...
对Argyris单元，其对偶基底中的法向导数  阻止了它成为仿射插值等价。 这样就阻止了将节点基底构建在参考胞上并且仿射映射的做法。 Domínguez和Sayas的最新工作（2008）开发了一种可纠正此问题的转换，与直接在网格中的每个单元上形成基底相比，所需的计算工作量更少。 可以将Argyris单元素推广到高于五次的多项式，依然给出$C^1$连续性，并且在顶点处$C^2$连续（Šolín等人，2004）。
