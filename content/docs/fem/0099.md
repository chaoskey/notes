...
如前所述，可以根据已经算出的有限元解$u_h$， 基于V-范数来估计误差， 或者基于输出函数来估计残差$r$的大小。 这可以以几种不同的方式来完成。 该估算通常涉及各部分的集成，以恢复原始PDE的逐元强残差，可能要与胞或拼接在一起的胞组上的局部问题的解相结合。 对于泊松方程（2.1）的标准分段线性有限元逼近，可以得到以下估计：

$$
\|u − u_h\|_V \equiv \|\nabla e\|_{L^2} \le C \left(\sum_{T \in \mathcal{T}_h} h^2_T\|R\|^2_T + h_T\|[\partial_n u_h]\|^2_{\partial T} \right)^{1/2}  \tag{2.79}
$$

其中$R|_T = f|_T+\Delta u_h|_T$是强残差，$h_T$表示网格大小（每个胞T周围最小外接球的直径），$[\partial_n u_h]$表示法向导数跨过网格维面（facet）的跳跃 。有关此估计的推导，请参见例如 Elman et al. (2005)。 令$\eta_T^2= h^2_T \|R\|^2_T + h_T \|[\nabla_n u_h]\|^2_{\partial T}$，我们得到的估计：

$$
\|u - u_h\|_V \le \eta_h \equiv C \left(\sum_T {\eta^2_T}\right)^{1/2} \tag{2.80}
$$

自适应算法试图确定网格大小$h=h(x)$，使得$\eta_h \le \epsilon$。 从初始的粗网格开始，逐个检查每个胞，如果误差指示器$\eta_T$比较大，那么细化该网格。 有几种可用的策略，例如细化所有$\eta_T$较大的胞的上半部分，即按$\eta_T$的大小排序的所有胞的前20％。 其他策略包括细化所有$\eta_T$高于$\max_{T\in \mathcal{T}_h}\eta_T$特定比例(比如：>MAX10%)的胞，或细化所有胞的最高特定比例（比如：TOP10%），以使它们的误差指标之和占$\eta_h$的重要部分（所谓的Dörﬂer标记（Dörﬂer （1996））。
