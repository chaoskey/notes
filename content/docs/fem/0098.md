...
- 区域T是$\mathbb{R}^d,\quad d=1,2,3,\dots$的有界封闭子集，具有非空的内部和分段光滑边界；

- 空间$\mathcal{V} = \mathcal{V}(T)$是n维区域T上的有限维函数空间；
...
例如，考虑上图中三角形上的标准线性拉格朗日有限元。这个单元格T由三角形给定，空间$\mathcal{V}$则由是T（维数为3的空间）上的一阶多项式的空间给定。作为$\mathcal{V}'$的基底，我们可以指定其为T的三个顶点值；那是：

$$
\begin{aligned}\mathcal{l}_i : \mathcal{V} \to \mathbb{R} \\  \mathcal{l}_i(v) = v(x^i) \end{aligned}  \tag{2.40}
$$

对于$i = 1,2,3$，其中$x_i$是第i个顶点的坐标。为了确认这确实是有限元，我们需要验证$\mathcal{L}$是$\mathcal{V}'$的基底。这等效于$\mathcal{L}$的唯一性。也就是说，如果$v \in \mathcal{V}$并且$\mathcal{l}_i(v)= 0,\quad \forall l_i$，那么有$v = 0$（Brenner and Scott，2008）。对于线性拉格朗日三角形，我们注意到如果v在每个顶点为零，那么则v在任何地方都必须为零，因为一个平面由其在三个非共线点的值唯一地确定。由此可见，线性拉格朗日三角形确实是一个有限元。一般而言，确定$\mathcal{L}$的唯一性可能并非易事。

### 2.4.3 节点基底

通过引入$\mathcal{V}$的节点基底，可以大大简化用$V_h$表示局部函数空间$\mathcal{V}$的基函数的有限元解。$\mathcal{V}$的节点基底$\{\phi_i\}^n_{i=1}$是$\mathcal{V}$的基底，若满足：
...
特别地，对于线性拉格朗日三角形，$\mathcal{V}$中的任何函数v可由$v = \sum^3_{i=1}{v(x^i)\phi_i}$给出。换句话说，对任何函数v，可以通过计算v处$\mathcal{L}$中的线性泛函值来获得v的展开系数。因此，我们可以将$u_h$的展开系数U和$\mathcal{L}$的线性泛函互相称为对方的自由度。
...
对于任何有限元$(T, \mathcal{V}, \mathcal{L})$，可以通过求解大小为$n \times n$的线性方程组来计算节点基底。 为了看出这一点，令$\{\psi_i\}^n_{n=1}$是$\mathcal{V}$的任意基底（主基底）。如果$\mathcal{V}$是一个完整的多项式空间，或者可以通过奇异值分解或Gram-Schmidt程序计算得出，则这样的基底很容易构造。参见Kirby（2004）。然后，我们可以以主基底项为节点基底建立Ansatz：
...
对于任何$v\in V_h$，因此，每个局部自由度$\mathcal{l}^T_i \int \mathcal{L}_T$对应于由局部到全局映射$\iota_T$确定的全局自由度$\mathcal{l}_{\iota_T(i)} \in \mathcal{L}$。正如我们将看到的，局部到全局的映射以及自由度的选择决定了全局函数空间$V_h$的连续性。

对于标准连续分段线性，可以通过简单地将$i=1,2,3$的每个局部顶点数i映射到相应的全局顶点数$\iota_T(i)$来定义局部到全局的映射。 对于连续的分段二次，可以将局部到全局的映射基于全局顶点和边号，如图下图所示，用于由两个三角形组成的简单网格。

![](../images/0181.jpg)


<center>图2.4 由两个三角形组成的简单网格的局部到全局映射。 左三角形（T）的六个局部自由度被映射到全局自由度$\iota_T(i)=1,2,4,4,9,8,5  \quad \mathrm{for} \ i = 1,2,\dots,6$。并且右三角形（T'）的六个局部自由度则被映射到全局自由度$\iota_{T'}(i)=2,3,4,7,9,6  \quad \mathrm{for} \ i = 1,2,\dots,6$。</center>
...
即，对每个单元格T，v的限制位于局部函数空间$\mathcal{V}_T$中。其次，我们要求对于任何一对单元格$(T,T') \in \mathcal{T}_h \times \mathcal{T}_h$和任何一对$(i, i') \in [1,n_T] \times [1, n_{T'}]$都满足：
