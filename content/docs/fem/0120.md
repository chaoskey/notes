...
这两对路线在数学上是等效的，每对路线的结果都是变量`A`代表张量$A$，变量`Aij`代表张量$A_{ij}$。 请注意，自由索引没有顺序，因此它们在表达式`v[j]*u[i]`中的出现顺序微不足道。 可以使用专用函数`as_vector`和`as_matrix`代替`as_tensor`。 尽管上面的示例使用了2阶张量，但是映射可推广到任意阶张量。
